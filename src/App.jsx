...
export default function App() {
  ...
  function applySessionToLocal(session) {
    ...
    if (session.dealId !== knownDealIdRef.current) {
      ...
      const startsAt = Date.now() + PREP_SECONDS * 1000;
      const endsAt = startsAt + sTurnSeconds * 1000;
      setGameStartsAt(startsAt);
      setTurnEndsAt(endsAt);
      // Reset the display immediately; otherwise the timeout effect can see
      // the previous player's 0 and fire on the host before the first tick.
      setTimeLeft(sTurnSeconds);
  ...
  async function createQuickHost() {
    ...
    await writeSession(code, {
      code, status: 'waiting', turnSeconds: 10, startCards: 10, communityMode: false, objectionSeconds: 5,
    ...
    const botTimer = setTimeout(() => {
      try {
        ...
        if (found) {
          ...
          setMessageType('info');
          setCurrentIdx(0);
          setTurnEndsAt(Date.now() + turnSeconds * 1000);
  ...
  function applyPenaltyAndAdvance(reasonText) {
    ...
    const newTurnEndsAt = Date.now() + turnSeconds * 1000;
